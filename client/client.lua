...
RegisterNUICallback('copyVector3', function(data, cb)
  local coords = GetEntityCoords(PlayerPedId())
  local vectorString = string.format("vector3(%f, %f, %f)", coords.x, coords.z, coords.y)
  cb(vectorString)
  toggleNuiFrame(false)
end)
...
RegisterNUICallback('copyVector4', function(data, cb)
  ...
  local vectorString = string.format("vector4(%f, %f, %f, %f)", coords.x, coords.z, coords.y, heading)
  cb(vectorString)
  toggleNuiFrame(false)
end)
